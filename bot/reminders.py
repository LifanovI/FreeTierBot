...
def create_reminder(chat_id, text, next_run, interval=None, reminder_type='reminder'):
...
def get_reminders(chat_id):
    ...
    docs = db.collection('reminders').where('chat_id', '==', chat_id).where('active', '==', True).stream()
...
def mark_reminder_sent(reminder_ref, interval=None):
    ...
    if interval:
        ...
        reminder_ref.update({'active': False})
